// Local dev-server backend for Design Core, split out of vite.config.js.
// These plugins add the /api/* endpoints (project admin, users, prefs, summary,
// PNG render, image proxy, capture stream) and the public/data live-reload
...
function inferGithubPagesRootFromGit() {
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        const fp = resolve(root, "public/data/site.json");
        let disk = {};
        if (existsSync(fp)) {
          try {
        ...
        const inferred = normalizePublicBaseUrl(inferGithubPagesRootFromGit());
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        if (req.url === "/api/capture/reset-session" && req.method === "POST") {
          const browserDataDir = resolve(root, ".capture-browser-data");
          try {
