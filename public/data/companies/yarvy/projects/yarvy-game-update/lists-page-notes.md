...
- **C3. Clean edge.** A review pass on C2. Counts become bare numbers again, flush right in one column, with a 44px disclosure column after them that only parents fill (Work's caret, and Archived's caret now sits there too instead of leading). The per row More menu leaves; list actions open by long press or from the list page's More. The header list-plus is removed, so the Your lists disc is the one add-list control and the FAB adds tasks and notes. Favorites stays as a section, and the list's own row in Your lists gets a small star so the second row reads as the same list. Inbox alone gets an accent pill, since it is the one count that asks for action. Rows sit at the 44px minimum, and section heads share one height, so the whole page, Archived included, clears the FAB at rest. The sublist indent is 34px, so the child icon lines up under the parent's name. Parent counts cover only the list's own tasks. Cost: without the kebab, actions are hidden until someone long presses, so this needs the same discoverability test C does.
